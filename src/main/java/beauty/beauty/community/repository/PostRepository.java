...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("SELECT p FROM Post p JOIN FETCH p.user WHERE (:lastId IS NULL OR p.id < :lastId) ORDER BY p.id DESC LIMIT :size")
    List<Post> findAllCursor(@Param("lastId") Long lastId, @Param("size") int size);

    // [Flow 2] 우리동네 탭 커서 쿼리: district 필터 추가
    @Query("SELECT p FROM Post p JOIN FETCH p.user WHERE p.district = :district AND (:lastId IS NULL OR p.id < :lastId) ORDER BY p.id DESC LIMIT :size")
